...
if __name__ == '__main__':
    ...
    # Load nucleolus features and get problematic nucleolus masks
    nucleoli_to_remove = (
        pl
        .read_csv(args.qc_compartment_features)
        .filter(
            (pl.col('Nucleolus_AreaShape_Area') >= 400) |
            (pl.col('Nucleolus_Parent_Nuclei') == 0)
        )
        .select(["Cell_ID", "Nucleolus_Number_Object_Number"])
    )
    ...
    delete_problematic_compartment_masks(
        ...
        filtered_comps=nucleoli_to_remove,
        comp_name="Nucleolus",
