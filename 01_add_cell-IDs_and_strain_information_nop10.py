...
parser.add_argument('-x', '--coordinates_ts1', default='', help='Path to file with coordinates for each ORF/strain (rep TS1).')
parser.add_argument('-y', '--coordinates_ts23', default='', help='Path to file with coordinates for each ORF/strain (reps TS2/TS3).')
...
def add_cell_info(path_to_db, comp_identifier, coordinates_path_ts1, coordinates_path_ts23, is_tsa='False', leading_zeroes=3):
    """
    ...
    Args:
        ...
        coordinates_path_ts1 (str): path to csv file with array mapping for ORFs/strains on plate (TS1)
        coordinates_path_ts23 (str): path to csv file with array mapping for ORFs/strains on plate (TS2 and TS3)
    ...
    # For Nop10 TS, TS1 has a different coordinates mapping sheet than TS2/TS3. Here, these mapping sheets are merged
    # before being joined with the main dataframe.
    coordinates_ts1 = (
        pl
        .read_csv(coordinates_path_ts1)
        .with_columns(
            pl.lit("TS1").alias("Replicate"),
    ...
    coordinates_ts23 = (
        pl
        .read_csv(coordinates_path_ts23)
        .with_columns(
            pl.col("Plate").cast(pl.String).str.zfill(2).alias("Plate"),
            pl.col("Row").cast(pl.String).str.zfill(leading_zeroes).alias("Row"),
            pl.col("Column").cast(pl.String).str.zfill(leading_zeroes).alias("Column")
    ...
    coordinates_ts3 = (
        coordinates_ts23
        .with_columns(
            pl.lit("TS3").alias("Replicate")
        )
    )

    coordinates = pl.concat([coordinates_ts1, coordinates_ts2, coordinates_ts3], how="vertical")
    ...
    combined_df = (
        ...
        .join(coordinates, on=["Replicate", "Plate", "Row", "Column"], how="left"))  # add ORF/marker information
...
if __name__ == '__main__':
    add_cell_info(
        ...
        coordinates_path_ts1=args.coordinates_ts1,
        coordinates_path_ts23=args.coordinates_ts23,
