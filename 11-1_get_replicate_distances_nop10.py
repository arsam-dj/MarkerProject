...
def modify_nop10_strain_coords(strain_dataframe):
    """
    Nop10 is different from other screens because:
        1) TS1 has Plates 04/05 while other TS reps don't
        2) The strain mapping sheet for TS1 is different from TS2/3
    This messes with a lot of the following code that groups strains based on Plate (it assumes that all three replicates
    have the exact same plates and same mapping sheet). This function addresses this problem by re-writing TS1 strain coordinates
    so they match TS2/3. Original files and databases are not modified; just the dataframe loaded within this script.
    
    Args:
        strain_dataframe (pl.DataFrame): table with strain information and coordinates
    ...
    ts1_plates_remapped = (
        ...
        .select(unchanged_plates.columns)
    )
...
def get_per_rep_pens_cell_counts_and_distances(database_directory, outlier_cells_directory, screen, output_directory):
    ...
    # remap coordinates for Nop10 TS1
    outlier_cell_counts = modify_nop10_strain_coords(outlier_cell_counts)
    ...
    # remap coordinates for Nop10 TS1
    # essentially what I'm doing is, merging TS1 Plate04/05 with Plate01/02/03. Imagine pouring all cells from a Plate04/05
    # well into a Plate01/02/03 well (for the same strain ID). Not exactly advisable, but it's the simplest way of accounting
    # for the fact the Nop10 mapping sheets for TS1 and TS2/3 differ.
    merged_cell_counts = modify_nop10_strain_coords(merged_cell_counts)
    
    # continue
    merged_cell_counts = (
        merged_cell_counts
        .group_by(["Plate", "Replicate", "Row", "Column", "ORF", "Name", "Strain_ID"]) # this is where Plate04/05 gets merged with Plate01/02/03
        .agg(pl.col("Total_Num_Cells").sum().alias("Total_Num_Cells"))
